...
function deleteSelectedForOrderBox(e) {
  ...
  for (var i = 0; i < cartItems.length; i++) {
    if (
      selectedEl.querySelector('h3').textContent ===
      cartItems[i].querySelector('h3').textContent
    ) {
      cartItemsContainer[0].removeChild(cartItems[i]);
    }
  }
  // var nodes = document.getElementsByClassName('orders');
...
function productsHandler() {
  ...
  cartItem.classList.remove('active');
  cartItemsContainer[0].append(checkOutBtn);
  if (cartItems.length === 0) {
    checkOutBtn.innerText =
      'You have no item on your cart if you wanna add click below!';
    checkOutBtn.style.marginTop = '400px';
